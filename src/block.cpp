...
void miner(int sockfd){
    ...
    json blockchain = Net::recv_blockchain(sockfd); 
    
    addr_t addr;
    std::string block = Net::recv_block(sockfd, &addr); 
    size_t len_sign;
    std::pair<unsigned char*, EVP_PKEY*> sign = Net::recv_sign(sockfd, &len_sign);
    ...
    if (blockchain.is_null()) {
        res2 = Block::check_block(blockchain, json::parse(block));
    }
    if (res and res2){
        log("signature is valid");
        Block_t bl = Net::proof_of_work(block);
        int res = Net::commit_block(sockfd, bl);
        Net::send_response(sockfd, res, &addr);
    } else 
...
void service(int sockfd){
    ...
    Net::accept_connection(sockfd);
}

void running(bool target){
    while(target){}
}
...
void node(int sockfd, port_t port){
    ...
    Net::connect_service(port, sockfd);
    std::string ports = Net::recv_ports(sockfd); 
    Net::convert_ports(ports);
    json blockchain = Net::recv_blockchain(sockfd);
    Net::create_block(sockfd, res);
    std::thread th(Net::recv_request, sockfd);
    th.detach();
    running(true);
}
